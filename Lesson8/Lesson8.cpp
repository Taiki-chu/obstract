...
void Keyboard() {
		...
		{
				int fake = _getch();
				if (jump.position_y == 0 && (fake == 'e' || fake == 'w'|| fake == 'r' || fake == '7' || fake == '8' || fake == '9')) {
					touch = fake;
				}
				if (( fake == 'd' || fake == '0') && bullet[2] == 0) {
...
void explanation() {
	...
	printf("\x1b[%d;%dH w or 7 : 1段階(1.8秒)", 3, 0);
	printf("\x1b[%d;%dH e or 8 : 2段階(1.4秒)", 4, 0);
	printf("\x1b[%d;%dH r or 9 : 3段階(1.0秒)", 5, 0);
	printf("\x1b[%d;%dH d or 0 : 砲撃(1発) Oは壊せますがMは壊せません（メタル状態）", 6, 0);
}
...
{
	explanation();
	printf("\x1b[%d;%dH 文字よけゲーム", base_y - 2, 0);
	printf("\x1b[%d;%dH z を入力すると始まるよ！", base_y, 0);
	int z = 'a';
	while (z != 'z') {
		if (z == -11)return 0;
		z = _getch();
	...
	printf("\x1b[%d;%dH   　　　　　　　　　　　 ", base_y, 0);
	printf("\x1b[%d;%dH   　　　　　　　　", base_y - 2, 0);
	...
	do {
		...
		if (bullet[2] == 1) {
			...
			if (bullet[1] >= 20) {
				printf("\x1b[%d;%dH ", bullet[0], bullet[1]);
				bullet[0] = 0;
				bullet[1] = 0;
				bullet[2] = 0;
			}
		...
		if( ( frame_count % 7 == 0 || ( random = rand() % 15 ) == 0 ) && obstacle.size() < OBSTACLE_SIZE ) {//7カウントに1回,1/15の確立で
			...
			o->setPosition(obstacle_first_x, rand() % AREA_SIZE_y );//x,yをセット 0~3まで
		...
		for (int i = 0; i < obstacle.size();) {
			...
			if ( obstacle[i]->getPosition().x == 0 ||
				...
				printf("\x1b[%d;%dH ", y + base_y, x+1);//------------------------------------------------------------------ひとつ前のobstacleを削除
				if (obstacle[i]->getmetal() == 1) {
					printf("\x1b[%d;%dH\x1b[33m\x1b[47m%c\x1b[49m\x1b[39m", y + base_y, x, obstacle[i]->getWord());//---------obstacleを描画（メタル）
				}
				else {
					printf("\x1b[%d;%dH\x1b[33m%c\x1b[39m", y + base_y, x, obstacle[i]->getWord());//------------obstacleを描画（ノーマル）
				}
	...
	for (int i = 0; i < base_y + AREA_SIZE_y; i++) {
		printf("\x1b[%d;%dH                                                                       ", i , 0 );//空白35
	}
